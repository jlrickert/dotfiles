...
local function save_and_quit_if_modified()
	save_if_modified()
	vim.cmd.quit()
end
...
local function create_keg_template(opt)
	vim.api.nvim_create_user_command(opt['command'], function()
		...
		local contents = opt['contents']()
		vim.fn.mkdir(node_path, "p") -- Create parent directories if they don't exist

		local file_contents = { title }
		for _, v in ipairs(contents) do
			table.insert(file_contents, v)
		end
		vim.fn.writefile(file_contents, node_fname) -- Create an empty file
...
M.setup = function()
	...
	-- vim.api.nvim_create_user_command('KegWrite', function()
	-- 	save_if_modified()
	-- end, { desc = "Update keg file" })

	local buf_hashes = {}
	-- vim.api.nvim_create_autocmd({ "BufWritePre" }, {
	-- 	pattern = { "*.md", "*.org" },
	-- 	desc = "Store the current hash of the buffer contents before save",
	-- 	callback = function()
	-- 		local bufnr = vim.api.nvim_get_current_buf()
	-- 		buf_hashes[bufnr] = get_file_hash(bufnr)
	-- 	end
	-- })
	--
	-- vim.api.nvim_create_autocmd({ "BufWritePost" }, {
	-- 	pattern = { "*.md", "*.org" },
	-- 	desc = "Compare buffer hashes and reset modified flag if unchanged",
	-- 	callback = function()
	-- 		local bufnr = vim.api.nvim_get_current_buf()
	-- 		local pre_write_hash = buf_hashes[bufnr]
	-- 		local post_write_hash = get_file_hash(bufnr)
	--
	-- 		if pre_write_hash == post_write_hash then
	-- 			-- If hashes are equal, the buffer content did not change
	-- 			vim.api.nvim_buf_set_option(bufnr, "modified", false)
	-- 		end
	--
	-- 		-- Clean up the stored hash for the buffer
	-- 		buf_hashes[bufnr] = nil
	-- 	end
	-- })

	-- vim.cmd("command! wg Wg")
end
